...
export function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b-[4px] border-foreground ${
    ...
    >
      <div className="container mx-auto px-3 sm:px-4 flex items-center justify-between gap-2 sm:gap-4">
        <div
          className={`flex items-center shrink-0 transition-all duration-300 ${scrolled ? "gap-2 md:gap-4" : "gap-0"}`}
        >
          <div
          ...
          <div className="flex flex-col">
            <span
              className={`font-headline ${
                !scrolled ? "text-lg" : "text-md"
              } md:text-2xl leading-none uppercase tracking-tighter sm:tracking-tight text-primary`}
            >
              Defendamos la Séptima
            </span>
            <span className="text-[12px] md:text-xs font-bold text-foreground opacity-80 uppercase tracking-widest mt-1 -mb-2 sm:mb-0">
              Comité Ciudadano
            </span>
          </div>
        </div>
